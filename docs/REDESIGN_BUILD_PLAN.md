...
|---|---|---|---|
| 1 | **Push migration `0002` to Neon** | DB schema changes require explicit approval (security rule). The migration file `src/db/migrations/0002_narrow_lady_ursula.sql` is generated and committed but **NOT applied**. The new `apiKeys`, `searchHistory`, `documents`, `notificationPreferences` tables + `users` columns don't exist in the live DB yet, so API-key/audit/documents/onboarding features will error at runtime until applied. | `pnpm drizzle-kit push` (review the SQL first). Back up Neon before pushing. |
